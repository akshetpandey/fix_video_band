...
from .cli import annotate_frame_candidates, detect_terminal, display_frame
from .lib import (
    Band,
    build_shots,
    detect_band,
...
def main() -> None:  # noqa: C901, PLR0912, PLR0914, PLR0915
    ...
    ap.add_argument(
        '--display-cols', type=int, default=120, help='Width of inline frame previews in terminal columns (iTerm2)'
    )
    args = ap.parse_args()
    ...
    console.print(
        Panel.fit(
            f'[bold cyan]fix_video[/]\n[dim]Input :[/]  {args.input}\n[dim]Output:[/]  {args.output}',
            border_style='cyan',
        )
    )
    ...
    for i, shot in enumerate(shots):
        ...
        console.print(
            f'\n  [bold]Shot {i + 1}/{len(shots)}[/]  [dim]{shot.t0:.2f}s → {shot.t1:.2f}s[/]  ({shot_dur:.2f}s)'
        )
        ...
        display_frame(ref_frame, cols=args.display_cols, terminal=terminal)
        print()  # noqa: T201
